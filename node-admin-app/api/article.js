...
router.get("/category/:category", async (req, res) => {
  try {
    ...
    if (articles.length === 0) {
      return res
        .status(404)
        .json({ error: "No articles found in this category" });
    }
...
router.get("/subcategory/:subcategory", async (req, res) => {
  try {
    ...
    if (articles.length === 0) {
      return res
        .status(404)
        .json({ error: "No articles found in this subcategory" });
    }
...
router.get("/writer/:writerId", async (req, res) => {
  ...
  try {
    ...
    if (articles.length === 0) {
      return res
        .status(404)
        .json({ error: "No articles found for this writer" });
    }
...
router.get("/search/:title", async (req, res) => {
  ...
  try {
    const articles = await db.Article.findAll({
      where: {
        title: {
          [db.Sequelize.Op.like]: `%${title}%`, // 제목에 검색어가 포함된 기사 검색
        },
      },
      include: [
        {
    ...
    if (articles.length === 0) {
      return res
        .status(404)
        .json({ error: "No articles found matching the title" });
    }
