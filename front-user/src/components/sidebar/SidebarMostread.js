...
function MostreadArticle({ article }) {
...
export default function MostreadArticles({ heading }) {
  const [articles, setArticles] = useState([]); // 기사를 저장할 상태를 선언함

  useEffect(() => {
    async function fetchArticles() {
      try {
        const response = await axios.get('/article/most-read-articles'); // 조회수 순으로 기사를 가져오기 위한 API 요청을 보냄
    ...
    <div className="w-full rounded-2xl bg-gray-50 p-5 sm:p-8">
      ...
      <div className="space-y-6 pt-6 sm:space-y-5 lg:space-y-6 xl:space-y-5">
        {articles.map((article, index) => (
          <MostreadArticle key={`mostread-article-${index}`} article={article} />
        ))}
      </div>
    </div>
  );
