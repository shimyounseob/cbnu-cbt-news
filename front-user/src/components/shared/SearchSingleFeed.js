...
// 검색한 기사들을 가져와서 표시하는 컴포넌트
export default function SearchSingleFeed({ searchWord }) {
  ...
  useEffect(() => {
    async function fetchArticles() {
      try {
        const response = await axios.get(`/article/search/${searchWord}`);
        ...
        setError('No Article Found'); // 에러 발생 시 메시지 설정
      }
    }
  ...
  if (error) {
    return (
      <div className="text-center text-xl font-medium text-gray-700 py-10">
        {error}
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-xl px-4 sm:px-6 md:max-w-3xl md:px-8 lg:max-w-none lg:px-0">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-1">
          {currentArticles.map((article, index) => (
            <Article
              key={`search-single-article-${index}`}
              article={article}
